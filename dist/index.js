...
// src/index.ts
var CATEGORY_NAME_TO_ID_MAP = {
  "Business": "ct_o9mjlawxowkdy3rp",
  "Marketing": "ct_6vqzjd52vw8xlep3",
  "Entrepreneurship": "ct_m68b3l5q85x7k4rp",
  "Startups": "ct_vy2zbpn3km5q3m7g",
  "Technology": "ct_3krv4dnrrqn79l6o",
  "Digital Marketing": "ct_o9mjlawxk9nkdy3r",
  "Leadership": "ct_ox4amd5dmy5eyq2r",
  "Management": "ct_pr6a94nganez2dk3",
  "Productivity": "ct_2v89gonyg4nrqj3d",
  "Professional Training": "ct_adpvjb57p957k6qe",
  "Career Development": "ct_r42ak6np8gwjgdqx",
  "Investing": "ct_7v9m4lnk9wx6jkap",
  "Non-Profit": "ct_l7zo4qnzowmvpybd",
  "Remote Work": "ct_rmxeo6n6265vqpzj",
  "Careers": "ct_8kgrblw8o5eo36zm",
  "FinTech": "ct_akrev35b49w4ypql",
  "Artificial Intelligence": "ct_pr6a94ngov5ez2dk",
  "Cyber Security": "ct_l7zo4qnzde5mvpyb",
  "Programming": "ct_z9majpn49o5o73bx",
  "Machine Learning": "ct_6vqzjd522m58xlep"
};
var PODSCAN_TOPIC_SEARCH_URL = "https://podscan.fm/api/v1/topics/search";
var PODSCAN_TOPIC_EPISODES_URL = "https://podscan.fm/api/v1/topics";
var PODSCAN_EPISODE_SEARCH_URL = "https://podscan.fm/api/v1/episodes/search";
  ...
  async init() {
    this.server.tool(
      ...
      async ({ searchTerm }) => {
        ...
        const url = `${PODSCAN_TOPIC_SEARCH_URL}?${params.toString()}`;
        try {
          const response = await fetch(url, {
            method: "GET",
            headers: {
              "Authorization": `Bearer ${this.env.PODSCAN_API_KEY}`,
              "Content-Type": "application/json",
          ...
          if (topics.length > 0) {
            return {
              content: [{ type: "text", text: JSON.stringify(topics[0]) }]
            };
          } else {
            return {
    ...
    this.server.tool(
      "searchEpisodes",
      {
        ...
        minPodcastAudienceSize: z.number().optional(),
        maxPodcastAudienceSize: z.number().optional(),
        region: z.string().optional(),
        perPage: z.number().optional(),
        maxEpisodeAgeDays: z.number().optional()
      },
      async ({ searchTerm, categories, minPodcastAudienceSize, maxPodcastAudienceSize, region, perPage, maxEpisodeAgeDays }) => {
        const params = new URLSearchParams({
          query: `"${searchTerm}"`,
          per_page: String(perPage || 2),
          order_by: "posted_at",
          order_dir: "desc",
          show_full_podcast: "true",
          podcast_region: region || "US"
        });
        if (minPodcastAudienceSize !== void 0) {
          params.append("min_podcast_audience_size", String(minPodcastAudienceSize));
        }
        if (maxPodcastAudienceSize !== void 0) {
          params.append("max_podcast_audience_size", String(maxPodcastAudienceSize));
        }
        if (categories?.length) {
          const ids = categories.map((c) => CATEGORY_NAME_TO_ID_MAP[c]).filter(Boolean);
          if (ids.length) {
            params.append("category_ids", ids.join(","));
          }
        }
        if (maxEpisodeAgeDays) {
          const since = new Date(
            Date.now() - Number(maxEpisodeAgeDays) * 864e5
          ).toISOString();
          params.append("since", since);
        }
        const url = `${PODSCAN_EPISODE_SEARCH_URL}?${params.toString()}`;
        try {
          const response = await fetch(url, {
            method: "GET",
            headers: {
    ...
    this.server.tool(
      ...
      async ({ topicId, categories, minPodcastAudienceSize, maxPodcastAudienceSize, region }) => {
        const params = new URLSearchParams({
          per_page: "3",
          show_full_podcast: "true"
        });
        if (minPodcastAudienceSize !== void 0) {
          params.append("podcast_audience_min", String(minPodcastAudienceSize));
        }
        if (maxPodcastAudienceSize !== void 0) {
          params.append("podcast_audience_max", String(maxPodcastAudienceSize));
        }
        if (categories?.length) {
          const ids = categories.map((c) => CATEGORY_NAME_TO_ID_MAP[c]).filter(Boolean);
          if (ids.length) {
        ...
        const url = `${PODSCAN_TOPIC_EPISODES_URL}/${topicId}/episodes?${params.toString()}`;
        try {
          const response = await fetch(url, {
            method: "GET",
            headers: {
    ...
    this.server.tool(
      ...
      async ({ topicId }) => {
        const url = `${PODSCAN_TOPIC_EPISODES_URL}/${topicId}/related`;
        try {
          const response = await fetch(url, {
            method: "GET",
            headers: {
...
var index_default = {
  async fetch(request, env2, ctx) {
    const url = new URL(request.url);
    const id = env2.MCP_OBJECT.idFromName("default");
    const obj = env2.MCP_OBJECT.get(id);
